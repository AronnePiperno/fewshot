...
class T3ALNet(nn.Module):
    def __init__(
        ...
        self.support_features = self.load_support_features()
        
        # Load average features
        self.avg_features = self.avg_support_features(self.support_features)
    ...
    def load_support_features(self):
        ...
        for class_name in self.cls_names:
            ...
            if os.path.isdir(class_folder_path):
                npy_files = glob.glob(os.path.join(class_folder_path, "*.npy"))
        ...
        return torch.tensor(stacked_features, dtype=torch.float32)
    
    def infer_pseudo_labels_new(self, image_features, features):
        ...
        score = image_features_avg @ features.T

        # average the scores every 5
        score = score.view(-1, 5).mean(dim=-1)
        print(score)
        ...
        # Select top-k scores
        _, index = torch.topk(score, self.topk)
        ...
        return index[0], score
    
    def avg_support_features(self, features):
        # avg every 5 features
        avg_features = []
        for i in range(0, len(features), 5):
            avg_features.append(features[i:i+5].mean(dim=0))
    ...
    def forward(self, x, optimizer):
        ...
        video_features = self.support_features.to(image_features.device)
        print(video_features.shape)   
        indexes, scores_to_return = self.infer_pseudo_labels_new(image_features, video_features)
        ...
        for _ in range(self.steps):
            ...
            # put all the variables value in a file
            '''with open("./variables.txt", "w") as f:
                f.write(f"self.model.logit_scale: {self.model.logit_scale}\n")
                f.write(f"similarity_p: {similarity_p}\n")
                f.write(f"similarity_n: {similarity_n}\n")
                f.write(f"similarity: {similarity}\n")
                f.write(f"image_features_p: {image_features_p}\n")
                f.write(f"image_features_n: {image_features_n}\n")
                f.write(f"image_features: {image_features}\n")
                f.write(f"tta_emb: {tta_emb}\n")
                f.write(f"features: {features}\n")
                f.write(f"before_optimization_parameters_image_encoder: {before_optimization_parameters_image_encoder}\n")
                f.write(f"before_optimization_image_projection: {before_optimization_image_projection}\n")
                f.write(f"before_optimization_text_projection: {before_optimization_text_projection}\n")
                f.write(f"before_optimization_parameters_text_encoder: {before_optimization_parameters_text_encoder}\n")
                f.write(f"before_optimization_logit_scale: {before_optimization_logit_scale}\n")
                f.write(f"pindices: {pindices}\n")
                f.write(f"nindices: {nindices}\n")
                f.write(f"image_features_p: {image_features_p}\n")
                f.write(f"image_features_n: {image_features_n}\n")
                f.write(f"image_features: {image_features}\n")
                f.write(f"similarity_p: {similarity_p}\n")
        ...
        '''with open("./output_text.txt", "w+") as f:
            ...
            f.write(f"pred_mask: {pred_mask}\n")
            f.write(f"gt_mask: {gt_mask}\n")
