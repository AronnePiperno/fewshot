...
class T3AL0Net(nn.Module):
    def __init__(
        ...
        # Original initialization remains the same
    ...
    def adapt_features(self, image_features, device):
        """
        Adapt features using a momentum-based moving average approach
        """
        # Normalize image features
        image_features = image_features / image_features.norm(dim=-1, keepdim=True)
        
        # Get initial predictions
        similarities = image_features @ self.avg_features.to(device).T
        confidences = torch.softmax(similarities / 0.3, dim=-1)
        
        # Find high confidence predictions
        max_conf, pred_classes = torch.max(confidences, dim=-1)
        high_conf_mask = max_conf > self.confidence_threshold
        
        if high_conf_mask.any():
            # Group features by predicted class
            for class_idx in range(self.num_classes):
                class_mask = (pred_classes == class_idx) & high_conf_mask
                if class_mask.any():
                    # Update class prototype using moving average
                    class_features = image_features[class_mask].mean(0)
                    self.avg_features[class_idx] = (
                        self.tta_momentum * self.avg_features[class_idx].to(device) +
                        (1 - self.tta_momentum) * class_features
                    )
                    # Normalize updated prototype
                    self.avg_features[class_idx] = (
                        self.avg_features[class_idx] /
                        self.avg_features[class_idx].norm()
                    )
    ...
    def forward(self, x):
        ...
        # Project features using CLIP model
        with torch.no_grad():
            image_features = image_features
        image_features = image_features.squeeze(0)
        
        # Apply test-time adaptation
        adapted_features = self.adapt_features(image_features, device=image_features.device)
        
        # Get predictions using adapted features
        indexes, scores = self.infer_pseudo_labels(adapted_features)
        class_label = self.inverted_cls[indexes.item()]
        
        if self.remove_background:
            adapted_features = adapted_features - adapted_features.mean(dim=0, keepdim=True)
        
        # Normalize features
        image_features_norm = adapted_features / adapted_features.norm(dim=-1, keepdim=True)
        
        # Get pseudolabel features
        pseudolabel_feature = self.avg_features[indexes].squeeze()
        pseudolabel_feature = pseudolabel_feature / pseudolabel_feature.norm(dim=-1, keepdim=True)
        pseudolabel_feature = pseudolabel_feature.to(adapted_features.device)
        
        # Compute similarity

        print(f"Image features: {image_features_norm.shape}")
        print(f"Pseudolabel features: {pseudolabel_feature.shape}")
        similarity = pseudolabel_feature @ image_features_norm.T
        ...
        # Process segments and create output
        ...
        pred_mask = torch.zeros(adapted_features.shape[0]).to(adapted_features.device)
        gt_mask = torch.zeros(adapted_features.shape[0]).to(adapted_features.device)
        
        if segments:
            segment_features = [torch.mean(adapted_features[seg[0] : seg[1]], dim=0) for seg in segments]
            segment_features = torch.stack(segment_features)
            pred, scores = self.compute_score(
                self.model,
                segment_features,
                self.avg_features.to(adapted_features.device),
            )
        ...
        # Reset average features to original state
        self.avg_features = deepcopy(self.original_avg_features)
