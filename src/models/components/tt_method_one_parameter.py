...
class T3ALNet(nn.Module):
    def __init__(
        ...
        self.optim = torch.optim.AdamW(
            [
                ...
                {"params": self.model.visual.proj, "lr": 0.00001},
    ...
    def forward(self, x, optimizer):
        ...
        video_features = self.avg_video_features.to(image_features.device) @ self.model.visual.proj
        ...
        indexes, return_scores = self.infer_pseudo_labels(image_features, self.fuse_features.to(image_features.device))
        ...
        for _ in range(self.steps):
            ...
            projected_video_features = original_video_features @ self.model.visual.proj
        ...
        with torch.no_grad():
            ...
            if segments:
                ...
                video_features = video_features.to(image_features[0].device) @ self.model.visual.proj
                video_features = F.normalize(video_features, dim=-1)
                #video_features = self.video_proj(video_features)
        ...
        with torch.no_grad():
            ...
            if self.image_projection:
                ...
                self.model.visual.proj.data = before_optimization_image_projection
