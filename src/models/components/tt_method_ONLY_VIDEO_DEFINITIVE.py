...
class VideoProjector(nn.Module):
    def __init__(self, model,dim=768, dropout=0.1):
        super().__init__()

        #self.proj_matrix = copy.deepcopy(model.visual.proj)

        # Initialize the projection with the identity matrix
        self.proj_matrix = nn.Parameter(torch.eye(dim))
...
class T3ALNet(nn.Module):
    def __init__(
        ...
        self.video_proj = VideoProjector(self.model, dim=768, dropout=0.1)
        

        self.optim = torch.optim.Adam(
            [
                {"params": self.model.text.text_projection},
                {"params": self.model.visual.proj, "lr": 0.00000001},
                {"params": self.video_proj.parameters(), "lr": 0.000001},
    ...
    def infer_pseudo_labels(self, image_features):
        ...
        for avg_feature in self.avg_features:
            ...
            avg_feature = self.video_proj(avg_feature)
    ...
    def forward(self, x, optimizer):
        ...
        for _ in range(self.steps):
            ...
            tta_emb = self.avg_features[indexes.item()].unsqueeze(0)
            tta_emb = tta_emb.to(image_features.device)
            #tta_emb = tta_emb @ self.model.visual.proj
            tta_emb = self.video_proj(tta_emb)
            
            features = image_features - self.background_embedding if self.remove_background else image_features
            
            tta_emb = tta_emb / tta_emb.norm(dim=-1, keepdim=True)
            features = features / features.norm(dim=-1, keepdim=True)
            tta_emb = tta_emb.to(features.device)
        ...
        with torch.no_grad():
            ...
            if segments:
                ...
                support_features = self.avg_features
                support_features = support_features.to(image_features[0].device)
                #support_features = support_features @ self.model.visual.proj
                image_features = torch.stack(image_features)

                pred, scores = self.compute_score(
                    image_features,
                    support_features.to(image_features.device),
                )
        ...
        if self.image_projection:
            ...
            self.model.visual.proj.data = copy.deepcopy(before_optimization_image_projection)
        

        '''with torch.no_grad():

            self.video_proj.proj_matrix.data = copy.deepcopy(before_optimization_video_projection)
