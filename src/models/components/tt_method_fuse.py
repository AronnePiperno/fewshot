...
class T3ALNet(nn.Module):
    def __init__(
        ...
        self.lambda_contrast = 0.7
        self.neg_noise_scale = 0.3
        self.margin = 0.4

                
        # Add a feature update rate parameter
        self.feature_update_rate = 0.2
        
        # Store original features for restoration
        self.original_features = self.avg_video_features.clone()
        
        # Add a feature memory bank for tracking updates
        self.feature_memory = nn.ParameterDict({
            'video_features': nn.Parameter(self.avg_video_features.clone(), requires_grad=True)
        })
        


        self.fusion = AttentionFusion(embed_dim=768)

        self.optim = torch.optim.Adam(
            [
                {"params": self.model.parameters()},
                {"params": self.fusion.parameters()},
                {"params": self.feature_memory.parameters()},
            ], 
    ...
    def compute_contrastive_loss(self, tta_emb, positive, negatives, margin=0.4):

            # Normalize embeddings
            anchor = F.normalize(tta_emb, p=2, dim=-1)
            positive = F.normalize(positive, p=2, dim=-1)
            negatives = F.normalize(negatives, p=2, dim=-1)

            pos_dist = torch.norm(anchor - positive, p=2, dim=-1)
            neg_dist = torch.norm(anchor - negatives, p=2, dim=-1) 
    ...
    def update_support_features(self, current_features, class_idx):
        """Update support features using exponential moving average"""
        with torch.no_grad():

            if current_features.dim() > 1:
                current_features = current_features.mean(dim=0)
            

            current_features = F.normalize(current_features, dim=-1)
            

            support_features = self.feature_memory['video_features'][class_idx]
            

            if support_features.dim() != current_features.dim():
                support_features = support_features.view(-1)
                current_features = current_features.view(-1)

            support_features = support_features.to(current_features.device)

            

            updated_features = (1 - self.feature_update_rate) * support_features + \
                             self.feature_update_rate * current_features

            updated_features = F.normalize(updated_features, dim=-1)
            

            updated_features = updated_features.view(support_features.shape)


            self.feature_memory['video_features'].data[class_idx] = updated_features

    def restore_original_features(self):
        """Restore the original support features"""
        with torch.no_grad():
            # Reset feature memory to original values
            self.feature_memory['video_features'].data = self.original_features.clone()
    ...
    def forward(self, x, optimizer):
        ...
        self.fuse_features = self.fusion(text_features, video_features)
        ...
        for _ in range(self.steps):
            ...
            tta_emb = text_features.to(image_features.device)
            video_features = self.avg_video_features[indexes.item()].to(image_features.device)

            tta_emb = F.normalize(text_features, dim=-1)
            video_features = F.normalize(video_features, dim=-1)


            video_features = video_features.unsqueeze(0)

            tta_emb = self.fusion(text_features, video_features)
            ...
            tta_emb = self.fusion(text_features, video_features.unsqueeze(0))
           ...
           # negatives = self.generate_negatives(video_features, indexes.item(), image_features.device)
            ...
            #check if self.feature_memory['video_features'][indexes.item()] is updated
        ...
        assert not torch.equal(
            self.feature_memory['video_features'][indexes.item()],
            self.original_features[indexes.item()],
        ...
        with torch.no_grad():
            text_features = self.compute_tta_embedding(class_label, image_features.device)
            video_features = self.feature_memory['video_features'][indexes.item()]
            ...
            if segments:
                ...
                video_features = self.feature_memory['video_features'].squeeze(0)
                video_features = video_features.to(image_features[0].device)

                text_features =  F.normalize(text_features, dim=-1)
                video_features = F.normalize(video_features, dim=-1)

                fuse_features = self.fusion(text_features, video_features)
