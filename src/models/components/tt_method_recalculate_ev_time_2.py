...
class T3ALNet(nn.Module):
    def __init__(
        ...
        self.avg_features = self.get_support_image_features(self.model)
    ...
    def get_support_image_features(self, model):
        ...
        for class_name in self.cls_names:
            self.feature_name_list.append(class_name)
            ...
            for video in os.listdir(class_folder_path):
                ...
                video_features = self.extract_video_features(video_path, model)
    ...
    def extract_video_features(self, video_path, model) -> np.ndarray:
    ...
    def compute_tta_embedding(self, class_label, device):
        ...
        tta_emb = self.avg_features[class_label].unsqueeze(0)
    ...
    def forward(self, x, optimizer):
        ...
        with torch.no_grad():
            ...
            tta_emb = tta_emb.to('cuda')
            image_features_norm = image_features_norm.to('cuda')
            ...
            if segments:
                ...
                support_features = self.get_support_image_features(self.model)
