...
from src.models.components.loss import ByolLoss, HybridLoss, TTALoss
import open_clip
...
class T3ALNet(nn.Module):
    def __init__(
        ...
        self.temperature.data.clamp_(0.1, 1.0)
    ...
    def compute_score(self, x, avg_features):
        ...
        with torch.no_grad():
            ...
            scores = dot_product / self.temperature
    ...
    def update_avg_features(self, current_features, class_label, momentum=0.99):
        """Update class prototypes with exponential moving average."""
        class_idx = self.cls_names[class_label]
        current_feature = current_features.mean(dim=0).detach()
        
        # Update with momentum
        self.avg_features[class_idx] = (
            momentum * self.avg_features[class_idx] 
            + (1 - momentum) * current_feature
        )
    ...
    def forward(self, x, optimizer):
        ...
        for step in range(self.steps):
            ...
            if self.ltype in ["BYOL", "BCE"]:
                ...
                logits = (image_features @ self.avg_features.T) /self.temperature
                tta_loss, loss_dict = self.tta_loss(
                    logits,
                    image_features
                )
                print(f"Step {step} - TTA Loss: {tta_loss.item():.4f}, Entropy: {loss_dict['entropy_loss']:.4f}, Pseudo: {loss_dict['pseudo_loss']:.4f}")
            else:
                raise ValueError(f"Not implemented loss type: {self.ltype}")
        ...
        #self.update_avg_features(image_features, class_label)
